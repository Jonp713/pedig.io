...
var fs = require('fs')
  ...
  , util = require('./util')
  , store = require('./store')
  , client = require('pedig.io-client')
...
function Manager (server, options) {
  ...
  this.log.info('pedig.io started');
};
...
Manager.prototype.handleRequest = function (req, res) {
  ...
  if (data.static || !data.transport && !data.protocol) {
    if (data.static && this.enabled('browser client')) {
      ...
      res.writeHead(200);
      res.end('Welcome to pedig.io.');

      this.log.info('unhandled pedig.io url');
    }
...
Manager.prototype.handleUpgrade = function (req, socket, head) {
  ...
  if (!data) {
    if (this.enabled('destroy upgrade')) {
      socket.end();
      this.log.debug('destroying non-pedig.io upgrade');
    }
